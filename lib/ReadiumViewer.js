require(['jquery', 'EpubLibrary', 'EpubReader'], function($, EpubLibrary, EpubReader){
	...
	var initialLoad = function(){
		...
		if (epubUrl){
			EpubReader.loadUI({epub: decodeURIComponent(epubUrl.replace("_%2F%2F", "%3A%2F%2F"))});
		}
		else{
			EpubLibrary.loadUI();
		}
	...
	$(window).on('readepub', function(e, url){
		...
		pushState({epub: url}, "Readium Viewer", '?epub=' + encodeURIComponent(url).replace("%3A%2F%2F", "_%2F%2F"));
	});
